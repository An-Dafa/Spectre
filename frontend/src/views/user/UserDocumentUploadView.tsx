...
export function UserDocumentUploadView({
  ...
  const availableClasses = UPLOAD_PRIVACY_CLASSES;
  const [activeClasses, setActiveClasses] = useState<string[]>(availableClasses);
  const disabledClasses = availableClasses.filter((className) => !activeClasses.includes(className));
  ...
  function selectFile(nextFile: File | null) {
    ...
    if (!isSupportedDocument) {
      setFile(null);
      setPreview("");
      setFileError("File must be JPG, PNG, WEBP, or PDF.");
      return;
    }
    ...
    <div className="view-stack user-upload-page">
      ...
      <section className="user-hero user-hero-compact upload-hero">
        <div className="user-hero-copy">
          ...
          <p>
            Upload identity documents, let Spectre protect sensitive areas, then share only the safe output.
          </p>
        </div>
      </section>
            ...
            <div className="upload-card-header">
              ...
              <details className="upload-settings-dropdown">
                ...
                <div className="upload-settings-menu">
                  ...
                  <Field label="Active Target Classes">
                    <ClassSelectionGrid
            ...
            >
              ...
              {preview ? (
                isPdf ? (
                  <object className="document-pdf-preview" data={preview} type="application/pdf" aria-label="PDF preview">
                    <div className="document-drop-empty">
                      ...
                      <span>PDF is ready for processing, including all pages.</span>
                    </div>
                  </object>
                ) : (
                ...
                <div className="document-drop-empty">
                  ...
                  <span>or click to choose JPG, PNG, WEBP, or PDF</span>
                </div>
              )}
            </label>
            ...
            <div className={`redacted-viewer${redactedUrl ? " has-file" : ""}`}>
              {redactedUrl ? (
                ...
                <div className="document-drop-empty muted-empty">
                  <EyeOff size={42} />
                  <strong>Redaction results will appear here</strong>
                  <span>Spectre will show the safe output after processing finishes.</span>
                </div>
              )}
            </div>
          </section>
        ...
        <div className="result-box success-box upload-result-summary">
          <div className="meta-row">
            ...
            <div className="meta-item">
              ...
              <strong>{detectionCount} objects</strong>
            </div>
            <div className="meta-item">
              <span>Redacted</span>
            ...
            <div className="meta-item">
              <span>Rejected by Guardrail</span>
              <strong>{rejectedCount} objects</strong>
            </div>
          </div>
          ...
          {rejectedCount > 0 && (
            <div className="alert-card warning" style={{ marginBottom: 0 }}>
              ...
              <div>
                <strong>Guardrail rejected {rejectedCount} candidates</strong>
                <p>
                  Detections suspected as drawings/sketches or lacking official document evidence are not redacted in
                  precision demo mode.
                </p>
              </div>
            </div>
      ...
      {result && !result.ok && (
        <div className="alert-card warning">
          ...
          <div>
            <strong>Failed to Process Document</strong>
            <p>{result.error?.message || "A system error occurred."}</p>
          </div>
        </div>
      )}
      ...
      {result?.ok && (
        <div className="alert-card success">
          ...
          <div>
            <strong>Safe output saved</strong>
            <p>
              The redacted result enters the Operational Zone, while the original document is encrypted in the
              Sovereign Vault. As a user, you cannot open the original. Access is only available through the admin
              authorization path.
            </p>
          </div>
        </div>
      ...
      {result && (
        <Collapsible title="Technical Details (JSON)">
