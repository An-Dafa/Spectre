...
function getZoneClass(zone?: string | null) {
  ...
  if (zone === "Admin Access API") return "danger";
...
function getZoneIcon(zone?: string | null) {
  ...
  if (zone === "Admin Access API") return <Landmark size={17} />;
...
export function AuditLogView({
  ...
  const summary = useMemo(() => {
    ...
    const sensitiveEvents = logs.filter((log) =>
      ["Admin Access API", "Sovereign Vault"].includes(String(log.zone ?? "")),
    ).length;
    ...
    <div className="view-stack audit-log-view">
      <section className="audit-hero-card">
        ...
        <div className="audit-hero-copy">
          ...
          <p>
            Audit Log helps investigators see who did what, when, in which zone, and which record was affected without
      ...
      <section className="audit-metric-grid">
        ...
        <div className="audit-metric-card">
          ...
          <small>Vault and admin access events</small>
        </div>
        <div className="audit-metric-card">
          <span>Actors</span>
          ...
          <Field label="Record ID">
            <label className="audit-search-field">
              ...
              <input
                ...
                placeholder="Search a specific record"
              />
            </label>
          </Field>
          <Field label="Zone">
      ...
      <section className="audit-console-grid">
        <Panel title="Event Timeline" eyebrow={`Menampilkan ${logs.length} event`} icon={<ShieldCheck />}>
          {logs.length === 0 ? (
            <div className="empty-state">No ada event audit yang sesuai filter.</div>
          ) : (
        ...
        <aside className="audit-detail-card">
          {selectedLog ? (
            ...
            <div className="audit-detail-empty">
              ...
              <span>Event details and JSON metadata will appear in this panel.</span>
            </div>
          )}
        </aside>
      </section>
