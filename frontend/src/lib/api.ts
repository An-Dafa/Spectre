...
export function createAdminAccessRequest(input: {
  ...
  return requestJson<Record<string, unknown>>(withParams("/api/admin/access-requests", params), {
    method: "POST",
    headers: { "X-Admin-Token": input.adminToken },
  });
}

export function approveAdminAccessRequest(input: { requestId: string; approvedBy: string; approverToken: string }) {
  const params = new URLSearchParams();
  params.set("approved_by", input.approvedBy);
  return requestJson<Record<string, unknown>>(
    withParams(`/api/admin/access-requests/${encodeURIComponent(input.requestId)}/approve`, params),
    { method: "POST", headers: { "X-Approver-Token": input.approverToken } },
  );
}

export function getAdminAccessRequest(input: { requestId: string; adminToken: string }) {
  return requestJson<Record<string, unknown>>(`/api/admin/access-requests/${encodeURIComponent(input.requestId)}`, {
    headers: { "X-Admin-Token": input.adminToken },
  });
}

export async function downloadAdminOriginal(input: {
  requestId: string;
  accessToken: string;
  adminToken: string;
}) {
  ...
  const response = await requestBlob(
    withParams(`/api/admin/access-requests/${encodeURIComponent(input.requestId)}/secure-original`, params),
    { headers: { "X-Admin-Token": input.adminToken } },
  );
...
export function getRequiredBackendEndpoints() {
  return [
    ...
    { method: "POST", path: "/api/redact", feature: "Admin document redaction pipeline" },
    ...
    { method: "POST", path: "/api/admin/access-requests", feature: "Create original access request" },
    { method: "POST", path: "/api/admin/access-requests/{request_id}/approve", feature: "Approve access request" },
    { method: "GET", path: "/api/admin/access-requests/{request_id}", feature: "Access request status" },
    { method: "GET", path: "/api/admin/access-requests/{request_id}/secure-original", feature: "One-time original download" },
